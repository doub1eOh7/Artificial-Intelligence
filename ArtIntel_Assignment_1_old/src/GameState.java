//Finish isValid method
//Some way to make sure the state is valid with no shapes outside area.
//Currently wont work because array indexes out of bounds for filledSpaces array.
...
public class GameState 
{
	...
	private byte[] state = new byte[22];
	...
	{
		filledSpaces = initFilledSpaces.clone();
	...
	public boolean fillSpace(Shape shape)
	{
		int id = 0;
		int size = 0;
		int x1, x2, x3, x4;
		...
		//This isn't working for assigning values...
		id = shape.getId();
		size = shape.getSize();
		x1 = shape.getX1();
		x2 = shape.getX2();
		...
		if(size == 3)
		{
			if(filledSpaces[x1 + state[2*id]][y1 + state[2*id + 1]])
			{
				System.out.println(id + ": " + (x1 + state[2*id]) + ", " + (y1 + state[2*id + 1]));
				return true;
			}
			else
				filledSpaces[x1 + state[2*id]][y1 + state[2*id + 1]] = true;
			if(filledSpaces[x2 + state[2*id]][y2 + state[2*id + 1]])
			{
				System.out.println(id + ": " + (x2 + state[2*id]) + ", " + (y2 + state[2*id + 1]));
				return true;
			}
			else
				filledSpaces[x2 + state[2*id]][y2 + state[2*id + 1]] = true;
			if(filledSpaces[x3 + state[2*id]][y3 + state[2*id + 1]])
			{
				System.out.println(id + ": " + (x3 + state[2*id]) + ", " + (y3 + state[2*id + 1]));
				return true;
			}
			else
				filledSpaces[x3 + state[2*id]][y3 + state[2*id + 1]] = true;
		}
		else if(size == 4)
		{
			if(filledSpaces[x1 + state[2*id]][y1 + state[2*id + 1]])
			...
			if(filledSpaces[x3 + state[2*id]][y3 + state[2*id + 1]])
			{
				...
				filledSpaces[x3 + state[2*id]][y3 + state[2*id + 1]] = true;
			if(filledSpaces[x4 + state[2*id]][y4 + state[2*id + 1]])
			{
				System.out.println(id + ": " + (x4 + state[2*id]) + ", " + (y4 + state[2*id + 1]));
				return true;
			}
			else
				filledSpaces[x4 + state[2*id]][y4 + state[2*id + 1]] = true;
		}
	...
	public void nextState(int index, int increment)
	{
		state[index] = (byte) (state[index] + increment);
	}
}
